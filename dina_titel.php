...
elseif (isset($_GET['produk_slug'])){
      $sql3 = mysql_query("select nama_produk from produk where produk_slug ='".$_GET['produk_slug']."'");
      $j3   = mysql_fetch_array($sql3);
      echo "$j3[nama_produk]";;
  }
else{
    $sql4 = mysql_query("select nama_website from identitas LIMIT 1");
    $j4   = mysql_fetch_array($sql4);
    echo "$j4[nama_website]"; 
}  
?>
